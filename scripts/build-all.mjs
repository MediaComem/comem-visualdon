...
const decks = [
  ...
  { src: 'cours/03-js-dom.md',             out: '03-js-dom' },
  ...
  { src: 'cours/09-cartographie-web.md',   out: '09-cartographie-web', exportWaitUntil: 'load' },
...
for (const { src, out, exportWaitUntil } of decks) {
  ...
  // For decks with live iframes, generate PDF separately with --wait-until load
  if (exportWaitUntil) {
